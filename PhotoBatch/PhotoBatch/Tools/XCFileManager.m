...
//  macApp
//
//  Created by qianjn on 2017/11/23.
...
+ (BOOL)moveItemAtPath:(NSString *)path toPath:(NSString *)toPath overwrite:(BOOL)overwrite error:(NSError *__autoreleasing *)error {
    ...
    if (overwrite) {
        // 移动文件
        isSuccess = [[NSFileManager defaultManager] moveItemAtPath:path toPath:toPath error:error];
    } else {
        // 复制文件
        isSuccess = [[NSFileManager defaultManager] copyItemAtPath:path toPath:toPath error:error];
    }
    return isSuccess;
}
